...
GET tracing 用于回答“Google VM 在首个分叉前读取了哪些环境属性”。它把 Browser
host 边界的属性读取写成 L1 `operation=get`，与既有 `call/set/construct` 及 L0
事件组成同一条因果 trace。
...
`max_events` 只限制 GET 事件，允许范围为 `1..=1_000_000`。达到上限后不再保留 GET，
...
当前回归覆盖：默认关闭、语义一致、缺失属性、硬预算、native/host 因果顺序、
Trusted Types、Promise microtask、drained timer、异常恢复、资源与并发调用。
