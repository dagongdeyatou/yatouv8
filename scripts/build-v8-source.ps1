...
& .\scripts\import-msvc-environment.ps1 -Architecture $v8BuildMsvcArchitecture
...
if ($isCross) {
    ...
    & .\scripts\import-msvc-environment.ps1 -Architecture $targetMsvcArchitecture
...
# Callers that package several CPython ABIs in one process can now reuse this
# completed V8/Cargo target tree instead of invoking the source build again.
$env:YATOU_V8_SOURCE_PREPARED_TARGET = $effectiveTarget
